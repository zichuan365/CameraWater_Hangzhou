...
void SavePPMFile(void* g_pRGBImageBuf,uint32_t ui32Width, uint32_t ui32Height)
{
    char szName[100] = {0};
    ...
    if(g_pRGBImageBuf != NULL)
    {
        ...
        fwrite(g_pRGBImageBuf, 1, 4024 * 3036 * 3, phImageFile);
        fclose(phImageFile);
        phImageFile = NULL;
...
void GX_STDC CaptureHander::OnFrameCallbackFun(GX_FRAME_CALLBACK_PARAM* pFrame)
{
    ...
    if (pFrame->status == GX_FRAME_STATUS_SUCCESS)
    {
        ...
        if(p->m_isColorCamera)
        {
            ...
            DxRaw8toRGB24((unsigned char*)pFrame->pImgBuf, p->g_pRGBImageBuf,
                          pFrame->nWidth, pFrame->nHeight, RAW2RGB_NEIGHBOUR, DX_PIXEL_COLOR_FILTER(1), false);

            //SavePPMFile(p->g_pRGBImageBuf,pFrame->nWidth, pFrame->nHeight);
...
bool DhCamera::ConnectCamera(GX_OPEN_PARAM *pOpenParam,std::queue<cv::Mat>& imaQue,float Exposure,float Gain)
{
    ...
    if(m_IsColor)
    {
        ...
        GX_VERIFY_EXIT(emStatus);
        emStatus = GXSetFloat(m_hDevice, GX_FLOAT_BALANCE_RATIO, 2.3);
        GX_VERIFY_EXIT(emStatus);
